...
export function PanelLayout({ sidebar, header, children, footer, className }: PanelLayoutProps) {
  return (
    <div data-slot="panel-layout" className={cn('flex h-full shadow-[0px_16px_32px_0px_rgba(0,0,0,0.1)] rounded-[4px] overflow-hidden', className)}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-background">
        {header}
        <div className="relative flex-1 overflow-y-auto min-h-0 bg-background px-4 pt-4 pb-24">
          {children}
        </div>
        {footer}
      </div>
